    ...
    <div class="<?php echo esc_attr($single_sidebar_css);?> fix-right-left">
        ...
        <div class="single-page-content">
            <article class="hentry">
                ...
                <!-- style-post quote-->
                <div class="style-post quote background-color-2">
                    <?php
                        if ( has_post_format( 'quote' ) ) {
                            preg_match('~<blockquote>(.*?)</blockquote>~is',$post->post_content , $matches_quote);
                            if(isset($matches_quote[0]))
                                echo $matches_quote[0];
                        }
                    ?>
                </div>
                <!-- style-post quote-->
                ...
                <div class="body-content">
                    <?php
                        //find if there is any <blockquote> in the post
                        if(preg_match('~<blockquote>(.*?)</blockquote>~is',get_the_content(), $matches)){
                            // if yes, remove it from content
                            $quote = $matches[0];
							$content =  preg_replace ('~<blockquote>(.*?)</blockquote>~is',' ', get_the_content_with_formatting());
                            echo do_shortcode($content);

                            // $quote can be used some where else
                        }
                    ?>
                     <?php
